...
namespace iQuest
{
    public partial class frmMain : Form
    {
        ...
        void LoadFiles()
        {
            ...
            Text = "iQuest v1.3 - loaded in " + Math.Round(loadtime.TotalMilliseconds, 0).ToString() + "ms";
        }
        ...
        public void ShowQuestList(string filter)
        {
            ...
            if (filter == "")
            {
                foreach (Quest quest in QuestFile.Quests)
                {
                    ...
                    lstQuests.Items.Add(string.Format("{0} - {1}", quest.ID, questname));
                }
            }
            else
            {
                foreach (Quest quest in QuestFile.Quests)
                {
                    try
                    {
                        string str2 = filter.Split(new char[] { '=' })[0];
                        string s = filter.Split(new char[] { '=' })[1];
                        string questname = QuestDialogDic.ContainsKey(quest.Title) ? QuestDialogDic[quest.Title] : quest.ID.ToString();
                        if ((str2 == "Name") && questname.ToLower().Contains(s.ToLower()))
                        {
                            lstQuests.Items.Add(string.Format("{0} - {1}", quest.ID, questname));
                        }
                        if ((str2 == "NPC") && (quest.StartingNPC.ToString() == s))
                        {
                            lstQuests.Items.Add(string.Format("{0} - {1}", quest.ID, questname));
                        }
                        if ((str2 == "Level") && isBetween(quest.MinLevel, quest.MaxLevel, int.Parse(s)))
                        {
                            lstQuests.Items.Add(string.Format("{0} - {1}", quest.ID, questname));
                        }
                        if ((str2 == "StartLevel") && (quest.MinLevel == int.Parse(s)))
                        {
                            lstQuests.Items.Add(string.Format("{0} - {1}", quest.ID, questname));
                        }
                        if ((str2 == "EndLevel") && (quest.MaxLevel == int.Parse(s)))
                        {
                            lstQuests.Items.Add(string.Format("{0} - {1}", quest.ID, questname));
                        }
                    }
                    ...
                    {
                        MessageBox.Show("Malformed Search.\n\n<exp>=<value>\n\nExp = Name, NPC, Level, StartLevel, EndLevel");
                        return;
                    }
                }
            }
            lblQuestCount.Text = string.Format("Quests: {0}", lstQuests.Items.Count);
        }

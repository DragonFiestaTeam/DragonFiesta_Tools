...
#else
            if (!double.TryParse(details.Body, NumberStyles.AllowDecimalPoint, CultureInfo.InvariantCulture, out double result)) return false;
#endif
...
#else
            int quotient = Math.DivRem(stringLength, safeWordLength, out int remainder);
#endif
